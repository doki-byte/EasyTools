...
+ **v2.2.9：** 1. 重新设计便携发包的布局; 2. 修复index目录遍历失败的BUG

+ **v2.2.8：** 1. 更新微信小程序反编译目录排列顺序; 2. 修复index目录遍历失败的BUG; 3. 修复旧版本drios数据库无法连接的BUG

+ **v2.2.7：** 1. 新增微信小程序官方查询接口; 2. 修复密码爆破模块telnet执行命令失败的bug; 3. jsfinder中新增VueCrack提取功能

+ **v2.2.6：** 1. 修复主机扫描闪退的bug; 2. 工具仓库新增右键打开终端目录; 3. 渗透测试结果新增搜索功能; 4. 修复小程序反编译名称查询不到的bug

+ **v2.2.5：** 1. 优化备忘笔记功能,支持显示非md文件,支持拖拽文件夹进行展示; 2. 优化端口扫描逻辑 3. 密码爆破模块新增mqtt协议，并支持对应命令执行

+ **v2.2.4：** 1. 修复批量存储桶扫描任务无法暂停的bug; 2. 优化备忘笔记功能，支持显示非md文件; 3. 全局敏感操作增加二次确认，避免出现误操作的情况

+ **v2.2.3：** 1. 修复零零信安接口无法使用的bug; 2. 修复渗透测试、信息收集模块右键菜单被遮挡的bug

+ **v2.2.2：** 1. 新增云上安全，支持阿里云、腾讯云、百度云、华为云等aksk的连接与账号接管; 2. 优化数据库管理，修复数据无法保存的bug; 3. 空间测绘新增零零信安接口; 4. 修复登录状态一直失效的bug

+ **v2.2.1：** 1. 修复随机生成密码缺少类型的bug; 2. 优化ssh连接文件传输，修复文件路径支持输入路径并进去; 3. 删除渗透测试各模块空白页面; 4. 删除iflow配置，现在直接使用系统管理中的ai大模型配置; 5. 地图测试新增google地图api

+ **v2.2.0：** 1. 修复数据库管理中数据展示，以及侧边栏搜索展开样式; 2. 优化主机扫描，目录扫描，空间测绘支持联动jsfinder; 3. 优化代理池功能; 4. 优化小程序反编译功能，简化修复逻辑

+ **v2.1.9：** 1. 修复mac下ssh文件管理无法打开的bug; 2. 修复数据库管理第一条信息无法删除的bug

+ **v2.1.8：** 1. 优化端口扫描规则，提高扫描效率; 2. 重新设计ssh、ftp界面，优化上传下载逻辑; 3. 优化简练助手中各数据库数据展示，支持双击查看数据详情并进行编辑

+ **v2.1.7：** 1. 修复空间测绘查询bug

+ **v2.1.6：** 1. 渗透测试模块新增druid利用功能; 2. 优化restmate界面显示逻辑; 3. 修复密码爆破模块部分协议无法直接执行命令的bug

+ **v2.1.5：** 1. 优化资源桶遍历功能，优化kkfileview预览逻辑; 2. 修复一个bug

+ **v2.1.4：** 1. 优化资源桶遍历功能，支持kkfileview预览; 2. 重构渗透测试模块，调用httpx提高性能与准确度; 3. 新增AI底座; 4. 新增空间测绘; 5. 新增AI版域名收集功能; 6. 优化微信小程序反编译目录获取; 7. 优化密码爆破功能

+ **v2.1.3：** 1. 修复一个bug

+ **v2.1.2：** 1. 优化密码爆破功能; 2. 优化 CLI 工具与 MCP 集成; 3. 优化便携代理功能; 4. 修复mac部分界面显示bug; 5. 新增浏览器js提取并发包功能; 6. ip归属地查询新增高精接口
