...
func (p *Proxy) startListening() Response {
	...
	// 如果是xui模式，使用HTTP代理监听逻辑
	if profile.Xui == "1" || profile.Xui == "2" {
		return p.startHTTPListening()
	}

	// 否则使用原来的SOCKS5监听逻辑
	return p.startSOCKS5Listening()
}

// 启动SOCKS5代理监听
func (p *Proxy) startSOCKS5Listening() Response {
	// 检查缓存代理数
...
// 启动HTTP代理监听
func (p *Proxy) startHTTPListening() Response {
	runtime.EventsEmit(p.ctx, "log_update", "[INF] x-ui模式：HTTP代理信息已保存到Excel文件，请直接使用生成的HTTP代理")
	runtime.EventsEmit(p.ctx, "log_update", "[INF] 使用方法：http://用户名:密码@地址:端口")

	return p.errorResponse("x-ui扫描完成，HTTP代理信息已保存到Excel")
}
