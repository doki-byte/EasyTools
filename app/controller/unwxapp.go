...
func (u *UnWxapp) RunUnWxapp(_ struct{}, packages []string, appid string, format bool) (string, error) {
	...
	if _, err := exec.LookPath("node"); err != nil {
		...
		return "", fmt.Errorf("Node.js未安装或不在PATH中")
	}
	...
	if err != nil {
		...
		return "", fmt.Errorf("获取当前工作目录失败")
	}
	log.Println("当前工作目录:", cwd)

	// 构建工具目录路径（相对于项目根目录）
	unwxappDir := filepath.Join(cwd, "EasyToolsFiles", "tools", "Unwxapp")
...
func (u *UnWxapp) OpenFolder(request OpenFolderRequest) error {
	path := request.Path
	// 标准化路径（处理 ~ 等特殊符号）
