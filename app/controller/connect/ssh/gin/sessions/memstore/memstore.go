...
import (
	"EasyTools/app/controller/connect/ssh/gin/sessions/securecookie"
	sessions2 "EasyTools/app/controller/connect/ssh/gin/sessions/sessions"
	"bytes"
	"encoding/base32"
	"encoding/gob"
	"fmt"
...
// not be decoded.
func (m *MemStore) Get(r *http.Request, name string) (*sessions2.Session, error) {
	return sessions2.GetRegistry(r).Get(m, name)
}
...
func (m *MemStore) New(r *http.Request, name string) (*sessions2.Session, error) {
	session := sessions2.NewSession(m, name)
	options := *m.Options
...
func (m *MemStore) Save(r *http.Request, w http.ResponseWriter, s *sessions2.Session) error {
	...
	http.SetCookie(w, sessions2.NewCookie(s.Name(), cookieValue, s.Options))
	return nil
}
