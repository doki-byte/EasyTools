...
func (s *server) processRequest(data []byte) (any, error) {
	...
	case agentRemoveIdentity:
		...
		if err := ssh2.Unmarshal(data, &req); err != nil {
			return nil, err
		}

		var wk wireKey
		if err := ssh2.Unmarshal(req.KeyBlob, &wk); err != nil {
			return nil, err
		}
	...
	case agentSignRequest:
		...
		return &signResponseAgentMsg{SigBlob: ssh2.Marshal(sig)}, nil
...
func parseConstraints(constraints []byte) (lifetimeSecs uint32, confirmBeforeUse bool, extensions []ConstraintExtension, err error) {
	for len(constraints) != 0 {
		...
		case agentConstrainExtension, agentConstrainExtensionV00:
			...
			if err = ssh2.Unmarshal(constraints, &msg); err != nil {
...
func parseRSAKey(req []byte) (*AddedKey, error) {
	var k rsaKeyMsg
	if err := ssh2.Unmarshal(req, &k); err != nil {
		return nil, err
	}
	if k.E.BitLen() > 30 {
...
func parseEd25519Cert(req []byte) (*AddedKey, error) {
	...
	pubKey, err := ssh2.ParsePublicKey(k.CertBytes)
	if err != nil {
		return nil, err
	}
	priv := ed25519.PrivateKey(k.Priv)
	cert, ok := pubKey.(*ssh2.Certificate)
	if !ok {
...
func parseRSACert(req []byte) (*AddedKey, error) {
	...
	if err := ssh2.Unmarshal(cert.Key.Marshal(), &rsaPub); err != nil {
...
func parseDSACert(req []byte) (*AddedKey, error) {
	...
	if err := ssh2.Unmarshal(cert.Key.Marshal(), &w); err != nil {
		return nil, fmt.Errorf("agent: Unmarshal failed to parse public key: %v", err)
	}
...
func parseECDSACert(req []byte) (*AddedKey, error) {
	...
	if err := ssh2.Unmarshal(cert.Key.Marshal(), &ecdsaPub); err != nil {
		return nil, err
	}
...
func (s *server) insertIdentity(req []byte) error {
	...
	if err := ssh2.Unmarshal(req, &record); err != nil {
		return err
	}
	...
	switch record.Type {
	case ssh2.KeyAlgoRSA:
		addedKey, err = parseRSAKey(req)
	case ssh2.KeyAlgoDSA:
		addedKey, err = parseDSAKey(req)
	case ssh2.KeyAlgoECDSA256, ssh2.KeyAlgoECDSA384, ssh2.KeyAlgoECDSA521:
		addedKey, err = parseECDSAKey(req)
	case ssh2.KeyAlgoED25519:
		addedKey, err = parseEd25519Key(req)
	case ssh2.CertAlgoRSAv01:
		addedKey, err = parseRSACert(req)
	case ssh2.CertAlgoDSAv01:
		addedKey, err = parseDSACert(req)
	case ssh2.CertAlgoECDSA256v01, ssh2.CertAlgoECDSA384v01, ssh2.CertAlgoECDSA521v01:
		addedKey, err = parseECDSACert(req)
	case ssh2.CertAlgoED25519v01:
