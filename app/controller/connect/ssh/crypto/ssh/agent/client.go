...
import (
	ssh2 "EasyTools/app/controller/connect/ssh/crypto/ssh"
	"bytes"
	"crypto/dsa"
	"crypto/ecdsa"
	"crypto/ed25519"
...
type Agent interface {
	...
	Sign(key ssh2.PublicKey, data []byte) (*ssh2.Signature, error)

	// Add adds a private key to the agent.
	Add(key AddedKey) error

	// Remove removes all identities with the given public key.
	Remove(key ssh2.PublicKey) error
...
type ExtendedAgent interface {
	...
	SignWithFlags(key ssh2.PublicKey, data []byte, flags SignatureFlags) (*ssh2.Signature, error)
...
func (k *Key) Verify(data []byte, sig *ssh2.Signature) error {
	pubKey, err := ssh2.ParsePublicKey(k.Blob)
	if err != nil {
...
func parseKey(in []byte) (out *Key, rest []byte, err error) {
	...
	if err := ssh2.Unmarshal(in, &record); err != nil {
		return nil, nil, err
	}

	var wk wireKey
	if err := ssh2.Unmarshal(record.Blob, &wk); err != nil {
		return nil, nil, err
	}
...
func (c *client) Remove(key ssh2.PublicKey) error {
	req := ssh2.Marshal(&agentRemoveIdentityMsg{
		KeyBlob: key.Marshal(),
	})
...
func (c *client) Unlock(passphrase []byte) error {
	req := ssh2.Marshal(&agentUnlockMsg{
		Passphrase: passphrase,
	})
	return c.simpleCall(req)
}
...
// Sign has the agent sign the data using a protocol 2 key as defined
// in [PROTOCOL.agent] section 2.6.2.
func (c *client) Sign(key ssh2.PublicKey, data []byte) (*ssh2.Signature, error) {
	return c.SignWithFlags(key, data, 0)
}

func (c *client) SignWithFlags(key ssh2.PublicKey, data []byte, flags SignatureFlags) (*ssh2.Signature, error) {
	req := ssh2.Marshal(signRequestAgentMsg{
		KeyBlob: key.Marshal(),
		Data:    data,
		Flags:   uint32(flags),
	})
	...
	case *signResponseAgentMsg:
		...
		if err := ssh2.Unmarshal(msg.SigBlob, &sig); err != nil {
			return nil, err
		}
...
func unmarshal(packet []byte) (any, error) {
	...
	if err := ssh2.Unmarshal(packet, msg); err != nil {
		return nil, err
	}
	return msg, nil
}
...
func (c *client) insertKey(s any, comment string, constraints []byte) error {
	...
	case *rsa.PrivateKey:
		...
		k.Precompute()
		req = ssh2.Marshal(rsaKeyMsg{
			Type:        ssh2.KeyAlgoRSA,
			N:           k.N,
	...
	case *dsa.PrivateKey:
		req = ssh2.Marshal(dsaKeyMsg{
			Type:        ssh2.KeyAlgoDSA,
			P:           k.P,
			Q:           k.Q,
			G:           k.G,
			Y:           k.Y,
	...
	case ed25519.PrivateKey:
		req = ssh2.Marshal(ed25519KeyMsg{
			Type:        ssh2.KeyAlgoED25519,
			Pub:         []byte(k)[32:],
			Priv:        []byte(k),
			Comments:    comment,
			Constraints: constraints,
...
func (c *client) Add(key AddedKey) error {
	...
	if secs := key.LifetimeSecs; secs != 0 {
		constraints = append(constraints, ssh2.Marshal(constrainLifetimeAgentMsg{secs})...)
	}
...
func (c *client) insertCert(s any, cert *ssh2.Certificate, comment string, constraints []byte) error {
	var req []byte
	switch k := s.(type) {
	case *rsa.PrivateKey:
		if len(k.Primes) != 2 {
	...
	case ed25519.PrivateKey:
		req = ssh2.Marshal(ed25519CertMsg{
			Type:        cert.Type(),
			CertBytes:   cert.Marshal(),
			Pub:         []byte(k)[32:],
			Priv:        []byte(k),
	...
	signer, err := ssh2.NewSignerFromKey(s)
	if err != nil {
		return err
	}
...
func (s *agentKeyringSigner) PublicKey() ssh2.PublicKey {
	return s.pub
}

func (s *agentKeyringSigner) Sign(rand io.Reader, data []byte) (*ssh2.Signature, error) {
	// The agent has its own entropy source, so the rand argument is ignored.
	return s.agent.Sign(s.pub, data)
}

func (s *agentKeyringSigner) SignWithAlgorithm(rand io.Reader, data []byte, algorithm string) (*ssh2.Signature, error) {
	...
	switch algorithm {
	case ssh2.KeyAlgoRSASHA256:
		flags = SignatureFlagRsaSha256
	case ssh2.KeyAlgoRSASHA512:
...
var _ ssh2.AlgorithmSigner = &agentKeyringSigner{}
...
var certKeyAlgoNames = map[string]string{
	ssh2.CertAlgoRSAv01:        ssh2.KeyAlgoRSA,
	ssh2.CertAlgoRSASHA256v01:  ssh2.KeyAlgoRSASHA256,
	ssh2.CertAlgoRSASHA512v01:  ssh2.KeyAlgoRSASHA512,
	ssh2.CertAlgoDSAv01:        ssh2.KeyAlgoDSA,
	ssh2.CertAlgoECDSA256v01:   ssh2.KeyAlgoECDSA256,
	ssh2.CertAlgoECDSA384v01:   ssh2.KeyAlgoECDSA384,
	ssh2.CertAlgoECDSA521v01:   ssh2.KeyAlgoECDSA521,
	ssh2.CertAlgoSKECDSA256v01: ssh2.KeyAlgoSKECDSA256,
	ssh2.CertAlgoED25519v01:    ssh2.KeyAlgoED25519,
	ssh2.CertAlgoSKED25519v01:  ssh2.KeyAlgoSKED25519,
}
...
func (c *client) Extension(extensionType string, contents []byte) ([]byte, error) {
	req := ssh2.Marshal(extensionAgentMsg{
