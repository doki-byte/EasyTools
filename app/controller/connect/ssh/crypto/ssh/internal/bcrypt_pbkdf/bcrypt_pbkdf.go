...
import (
	blowfish2 "EasyTools/app/controller/connect/ssh/crypto/blowfish"
	"crypto/sha512"
	"errors"
)
...
func bcryptHash(out, shapass, shasalt []byte) {
	c, err := blowfish2.NewSaltedCipher(shapass, shasalt)
	if err != nil {
		panic(err)
	}
	for i := 0; i < 64; i++ {
		blowfish2.ExpandKey(shasalt, c)
		blowfish2.ExpandKey(shapass, c)
	}
	copy(out, magic)
	for i := 0; i < 32; i += 8 {
		for j := 0; j < 64; j++ {
