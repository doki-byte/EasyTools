...
import (
	...
	"EasyTools/app/controller/connect/ssh/app/utils"
	ssh2 "EasyTools/app/controller/connect/ssh/crypto/ssh"
	gin2 "EasyTools/app/controller/connect/ssh/gin"
	"EasyTools/app/controller/connect/ssh/sftp"
	websocket2 "EasyTools/app/controller/connect/ssh/websocket"
	"encoding/json"
	"fmt"
	"io"
	"log/slog"
...
func (s *SshConn) connect(clientIp string) error {
	...
	config := ssh2.ClientConfig{
		...
		Auth: []ssh2.AuthMethod{
			ssh2.Password(s.Pwd),
			ssh2.KeyboardInteractive(func(name, instruction string, questions []string, echos []bool) ([]string, error) {
	...
	if s.AuthType == "cert" {
		...
		if s.CertPwd != "" {
			// 使用证书有证书密码登陆
			signer, err := ssh2.ParsePrivateKeyWithPassphrase(privateKeyBytes, privateKeyPassword)
			if err != nil {
				slog.Error("解析带密码私钥Key错误:", "err_msg", err.Error())
				return err
			}
			config.Auth = []ssh2.AuthMethod{
				ssh2.PublicKeys(signer),
			}
		} else {
			// 使用证书空密码登陆
			signer, err := ssh2.ParsePrivateKey(privateKeyBytes)
			if err != nil {
	...
	sshClient, err := ssh2.Dial(s.NetType, addr, &config)
	if err != nil {
		return err
	}
...
// RunTerminal 运行一个终端
func (s *SshConn) RunTerminal(shell string, stdout, stderr io.Writer, stdin io.Reader, w, h int, ws *websocket2.Conn) error {
	defer func() {
	...
	if err := s.sshSession.RequestPty(s.PtyType, h, w, modes); err != nil {
		...
		_ = websocket2.Message.Send(ws, "sshSession.RequestPty error:"+err.Error())
		return err
	}
	...
	if err != nil {
		...
		_ = websocket2.Message.Send(ws, "sshSession.Run error:"+err.Error())
		return err
	}
	return nil
}
...
// ResizeWindow  调整终端大小
func (s *SshConn) ResizeWindow(c *gin2.Context) {
	defer func() {
		if err := recover(); err != nil {
	...
	if !ok || cli == nil {
		c.JSON(200, gin2.H{"code": 1, "msg": "the client is disconnected"})
		return
	}
	...
	if !ok || conn == nil {
		...
		c.JSON(200, gin2.H{"code": 1, "msg": "to type SshConn error"})
		return
	}
	...
	c.JSON(200, gin2.H{"code": 0, "data": str, "msg": "ok"})
	return
}

func ResizeWindow(c *gin2.Context) {
	var sshObj = SshConn{}
	sshObj.ResizeWindow(c)
}

func NewSshConn(c *gin2.Context) {
	// WebSock 连接 SSH
	websocket2.Handler(func(ws *websocket2.Conn) {
		...
		if err != nil || (w < 40 || w > 8192) {
			_ = websocket2.Message.Send(ws, "connect error window width !!!")
			DeleteOnlineClient(sessionId)
			return
		}
		h, err := strconv.Atoi(ws.Request().URL.Query().Get("h"))
		if err != nil || (h < 2 || h > 4096) {
			_ = websocket2.Message.Send(ws, "connect error window height !!!")
			DeleteOnlineClient(sessionId)
			return
		}
		...
		cli, ok := OnlineClients.Load(sessionId)
		if !ok || cli == nil {
			_ = websocket2.Message.Send(ws, "session_id not exists !!!")
			DeleteOnlineClient(sessionId)
			return
		}
		...
		conn, ok := cli.(*SshConn)
		if !ok || conn == nil {
			_ = websocket2.Message.Send(ws, "to ssh.Session error !!!")
			DeleteOnlineClient(sessionId)
			return
		}
		err = conn.RunTerminal(conn.Shell, ws, ws, ws, w, h, ws)
		if err != nil {
			_ = websocket2.Message.Send(ws, "connect error:"+err.Error())
			DeleteOnlineClient(sessionId)
			return
		}
...
func CreateSessionId(c *gin2.Context) {
	var conn SshConn
	...
	if err != nil {
		c.JSON(200, gin2.H{"code": 1, "msg": "CreateSessionId error:" + err.Error()})
		return
	}

	OnlineClients.Store(sessionId, &conn)
	c.JSON(200, gin2.H{"code": 0, "data": sessionId, "msg": "ok"})
}

func Disconnect(c *gin2.Context) {
	defer func() {
		if err := recover(); err != nil {
			c.JSON(200, gin2.H{
				"code": 1,
...
func ExecCommand(c *gin2.Context) {
	type Param struct {
	...
	if !ok || cli == nil {
		c.JSON(200, gin2.H{"code": 3, "msg": "session not exists"})
		return
	}

	conn, ok := cli.(*SshConn)
	if !ok || conn == nil {
		c.JSON(200, gin2.H{"code": 4, "msg": "conn not exists"})
		return
	}
	...
	if err != nil {
		c.JSON(200, gin2.H{"code": 5, "msg": "create session error"})
		return
	}
	...
	if err != nil {
		c.JSON(200, gin2.H{"code": 6, "msg": "exec cmd error", "data": string(out)})
		return
	}
