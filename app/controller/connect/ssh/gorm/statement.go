...
type Statement struct {
	*DB
	TableExpr    *clause2.Expr
	Table        string
	Model                any
	Unscoped             bool
	Dest                 any
	ReflectValue reflect.Value
	Clauses      map[string]clause2.Clause
	BuildClauses []string
	Distinct             bool
...
func (stmt *Statement) QuoteTo(writer clause2.Writer, field any) {
	...
	switch v := field.(type) {
	case clause2.Table:
		if v.Name == clause2.CurrentTable {
	...
	case clause2.Column:
		if v.Table != "" {
			if v.Table == clause2.CurrentTable {
				write(v.Raw, stmt.Table)
			} else {
				write(v.Raw, v.Table)
			}
...
// AddVar add var
func (stmt *Statement) AddVar(writer clause2.Writer, vars ...any) {
	for idx, v := range vars {
		...
		case clause2.Column, clause2.Table:
		...
		case clause2.Interface:
			c := clause2.Clause{Name: v.Name()}
			v.MergeClause(&c)
			c.Build(stmt)
		case clause2.Expression:
			v.Build(stmt)
		case driver.Valuer:
		...
		case *DB:
			...
			if v.Statement.SQL.Len() > 0 {
				...
				if strings.Contains(sql, "@") {
					clause2.NamedExpr{SQL: sql, Vars: vars}.Build(subdb.Statement)
				} else {
					clause2.Expr{SQL: sql, Vars: vars}.Build(subdb.Statement)
				}
			} else {
...
func (stmt *Statement) AddClause(v clause2.Interface) {
...
func (stmt *Statement) AddClauseIfNotExists(v clause2.Interface) {
...
func (stmt *Statement) BuildCondition(query any, args ...any) []clause2.Expression {
	if s, ok := query.(string); ok {
		...
		if _, err := strconv.Atoi(s); err != nil {
			...
			if len(args) == 0 || (len(args) > 0 && strings.Contains(s, "?")) {
				...
				return []clause2.Expression{clause2.Expr{SQL: s, Vars: args}}
			}

			if len(args) > 0 && strings.Contains(s, "@") {
				// looks like a named query
				return []clause2.Expression{clause2.NamedExpr{SQL: s, Vars: args}}
			}
			...
			if len(args) == 1 {
				return []clause2.Expression{clause2.Eq{Column: s, Value: args[0]}}
			}
		}
	}

	conds := make([]clause2.Expression, 0, 4)
	...
	for idx, arg := range args {
			...
			if cs, ok := v.Statement.Clauses["WHERE"]; ok {
				if where, ok := cs.Expression.(clause2.Where); ok {
					if len(where.Exprs) == 1 {
						if orConds, ok := where.Exprs[0].(clause2.OrConditions); ok {
							where.Exprs[0] = clause2.AndConditions(orConds)
						}
					}
					conds = append(conds, clause2.And(where.Exprs...))
		...
		case map[any]any:
			for i, j := range v {
				conds = append(conds, clause2.Eq{Column: i, Value: j})
			}
		case map[string]string:
			...
			for _, key := range keys {
				conds = append(conds, clause2.Eq{Column: key, Value: v[key]})
			}
		case map[string]any:
			keys := make([]string, 0, len(v))
			for i := range v {
			...
			for _, key := range keys {
				...
				case reflect.Slice, reflect.Array:
					if _, ok := v[key].(driver.Valuer); ok {
						...
						conds = append(conds, clause2.IN{Column: key, Values: values})
					}
				default:
					conds = append(conds, clause2.Eq{Column: key, Value: v[key]})
				}
		...
		default:
			...
			if s, err := schema2.Parse(arg, stmt.DB.cacheStore, stmt.DB.NamingStrategy); err == nil {
					...
					for _, field := range s.Fields {
						...
						if selected || (!restricted && field.Readable) {
							if v, isZero := field.ValueOf(stmt.Context, reflectValue); !isZero || selected {
								if field.DBName != "" {
									conds = append(conds, clause2.Eq{Column: clause2.Column{Table: clause2.CurrentTable, Name: field.DBName}, Value: v})
								} else if field.DataType != "" {
									conds = append(conds, clause2.Eq{Column: clause2.Column{Table: clause2.CurrentTable, Name: field.Name}, Value: v})
								}
							}
						}
					}
				...
				if len(args) == 1 {
					...
					case reflect.Slice, reflect.Array:
						...
						if len(values) > 0 {
							conds = append(conds, clause2.IN{Column: clause2.PrimaryColumn, Values: values})
							return []clause2.Expression{clause2.And(conds...)}
						}
						return nil
					}
				}

				conds = append(conds, clause2.IN{Column: clause2.PrimaryColumn, Values: args})
			}
		}
	}
...
func (stmt *Statement) ParseWithSpecialTableName(value any, specialTableName string) (err error) {
	if stmt.Schema, err = schema2.ParseWithSpecialTableName(value, stmt.DB.cacheStore, stmt.DB.NamingStrategy, specialTableName); err == nil && stmt.Table == "" {
		if tables := strings.Split(stmt.Schema.Table, "."); len(tables) == 2 {
			stmt.TableExpr = &clause2.Expr{SQL: stmt.Quote(stmt.Schema.Table)}
			stmt.Table = tables[1]
			return
		}
...
func (stmt *Statement) Changed(fields ...string) bool {
	...
	changed := func(field *schema2.Field) bool {
...
func (stmt *Statement) SelectAndOmitColumns(requireCreate, requireUpdate bool) (map[string]bool, bool) {
	...
	processColumn := func(column string, result bool) {
		...
		} else if column == clause2.Associations {
