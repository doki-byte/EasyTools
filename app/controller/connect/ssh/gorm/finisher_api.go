...
func (db *DB) Save(value any) (tx *DB) {
	...
	case reflect.Slice, reflect.Array:
		if _, ok := tx.Statement.Clauses["ON CONFLICT"]; !ok {
			tx = tx.Clauses(clause2.OnConflict{UpdateAll: true})
		}
	...
	default:
		...
		if updateTx.Error == nil && updateTx.RowsAffected == 0 && !updateTx.DryRun && !selectedUpdate {
			return tx.Session(&Session{SkipHooks: true}).Clauses(clause2.OnConflict{UpdateAll: true}).Create(value)
		}
...
func (db *DB) First(dest any, conds ...any) (tx *DB) {
	tx = db.Limit(1).Order(clause2.OrderByColumn{
		Column: clause2.Column{Table: clause2.CurrentTable, Name: clause2.PrimaryKey},
	})
	if len(conds) > 0 {
		if exprs := tx.Statement.BuildCondition(conds[0], conds[1:]...); len(exprs) > 0 {
			tx.Statement.AddClause(clause2.Where{Exprs: exprs})
		}
	}
...
func (db *DB) FindInBatches(dest any, batchSize int, fc func(tx *DB, batch int) error) *DB {
	var (
		tx = db.Order(clause2.OrderByColumn{
			Column: clause2.Column{Table: clause2.CurrentTable, Name: clause2.PrimaryKey},
		}).Session(&Session{})
		queryDB      = tx
		rowsAffected int64
	...
	if c, ok := tx.Statement.Clauses["LIMIT"]; ok {
		if limit, ok := c.Expression.(clause2.Limit); ok {
			if limit.Limit != nil {
	...
	for {
		...
		queryDB = tx.Clauses(clause2.Gt{Column: clause2.Column{Table: clause2.CurrentTable, Name: clause2.PrimaryKey}, Value: primaryValue})
	}
	...
	for _, value := range values {
		...
		case []clause2.Expression:
			for _, expr := range v {
				...
				} else if andCond, ok := expr.(clause2.AndConditions); ok {
					db.assignInterfacesToValue(andCond.Exprs)
				}
			}
		case clause2.Expression, map[string]string, map[any]any, map[string]any:
...
func (db *DB) FirstOrInit(dest any, conds ...any) (tx *DB) {
	queryTx := db.Limit(1).Order(clause2.OrderByColumn{
		Column: clause2.Column{Table: clause2.CurrentTable, Name: clause2.PrimaryKey},
	})

	if tx = queryTx.Find(dest, conds...); tx.RowsAffected == 0 {
		if c, ok := tx.Statement.Clauses["WHERE"]; ok {
			if where, ok := c.Expression.(clause2.Where); ok {
...
func (db *DB) FirstOrCreate(dest any, conds ...any) (tx *DB) {
	tx = db.getInstance()
	queryTx := db.Session(&Session{}).Limit(1).Order(clause2.OrderByColumn{
		Column: clause2.Column{Table: clause2.CurrentTable, Name: clause2.PrimaryKey},
	})
	...
	if result.RowsAffected == 0 {
		...
		for i := 0; i < len(exprs); i++ {
			...
			if eq, ok := expr.(clause2.AndConditions); ok {
				exprs = append(exprs, eq.Exprs...)
			} else if eq, ok := expr.(clause2.Eq); ok {
				switch column := eq.Column.(type) {
				case string:
...
func (db *DB) Count(count *int64) (tx *DB) {
	...
	if len(tx.Statement.Selects) == 0 {
		tx.Statement.AddClause(clause2.Select{Expression: clause2.Expr{SQL: "count(*)"}})
	} else if !strings.HasPrefix(strings.TrimSpace(strings.ToLower(tx.Statement.Selects[0])), "count(") {
		expr := clause2.Expr{SQL: "count(*)"}
		...
		if len(tx.Statement.Selects) == 1 {
			...
			if len(fields) == 1 || (len(fields) == 3 && (strings.ToUpper(fields[1]) == "AS" || fields[1] == ".")) {
				...
				if tx.Statement.Distinct {
					expr = clause2.Expr{SQL: "COUNT(DISTINCT(?))", Vars: []any{clause2.Column{Name: dbName}}}
				} else if dbName != "*" {
					expr = clause2.Expr{SQL: "COUNT(?)", Vars: []any{clause2.Column{Name: dbName}}}
				}
			}
		}

		tx.Statement.AddClause(clause2.Select{Expression: expr})
	}
...
func (db *DB) Pluck(column string, dest any) (tx *DB) {
	...
	if len(tx.Statement.Selects) != 1 {
		...
		tx.Statement.AddClauseIfNotExists(clause2.Select{
			Distinct: tx.Statement.Distinct,
			Columns:  []clause2.Column{{Name: column, Raw: len(fields) != 1}},
		})
	}
	tx.Statement.Dest = dest
	return tx.callbacks.Query().Execute(tx)
...
func (db *DB) Exec(sql string, values ...any) (tx *DB) {
	...
	if strings.Contains(sql, "@") {
		clause2.NamedExpr{SQL: sql, Vars: values}.Build(tx.Statement)
	} else {
		clause2.Expr{SQL: sql, Vars: values}.Build(tx.Statement)
	}
