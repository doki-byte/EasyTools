...
func (m Migrator) FullDataTypeOf(field *schema2.Field) clause2.Expr {
...
func (m Migrator) AddColumn(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		if !f.IgnoreMigration {
			...
			columnName := clause2.Column{Name: f.DBName}
		...
		if stmt.Schema != nil {
			if field := stmt.Schema.LookUpField(field); field != nil {
				...
				return m.DB.Exec(
					...
					clause2.Table{Name: stmt.Table}, clause2.Column{Name: field.DBName}, fullDataType,
				).Error
			}
		}
...
func (m Migrator) RenameColumn(value any, oldName, newName string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		if field != nil {
			return m.DB.Exec(
				"ALTER TABLE ? CHANGE ? ? ?",
				clause2.Table{Name: stmt.Table}, clause2.Column{Name: oldName},
				clause2.Column{Name: newName}, m.FullDataTypeOf(field),
			).Error
		}
	...
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		if constraint != nil {
			...
			case *schema2.Constraint:
				return m.DB.Exec("ALTER TABLE ? DROP FOREIGN KEY ?", clause2.Table{Name: table}, clause2.Column{Name: name}).Error
			case *schema2.CheckConstraint:
				return m.DB.Exec("ALTER TABLE ? DROP CHECK ?", clause2.Table{Name: table}, clause2.Column{Name: name}).Error
			}
		}
		if m.HasIndex(value, name) {
			return m.DB.Exec("ALTER TABLE ? DROP INDEX ?", clause2.Table{Name: table}, clause2.Column{Name: name}).Error
		}
		return nil
	})
}
	...
	if !m.Dialector.DontSupportRenameIndex {
		return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
			return m.DB.Exec(
				...
				clause2.Table{Name: stmt.Table}, clause2.Column{Name: oldName}, clause2.Column{Name: newName},
			).Error
		})
	}
		...
		if stmt.Schema != nil {
			if idx := stmt.Schema.LookIndex(newName); idx == nil {
				if idx = stmt.Schema.LookIndex(oldName); idx != nil {
					...
					values := []any{clause2.Column{Name: newName}, clause2.Table{Name: stmt.Table}, opts}
...
func (m Migrator) DropTable(values ...any) error {
	...
	return m.DB.Connection(func(tx *gorm2.DB) error {
		tx.Exec("SET FOREIGN_KEY_CHECKS = 0;")
		for i := len(values) - 1; i >= 0; i-- {
			if err := m.RunWithValue(values[i], func(stmt *gorm2.Statement) error {
				return tx.Exec("DROP TABLE IF EXISTS ? CASCADE", clause2.Table{Name: stmt.Table}).Error
			}); err != nil {
				return err
			}
		}
...
func (m Migrator) ColumnTypes(value any) ([]gorm2.ColumnType, error) {
	...
	err := m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		var (
			...
			rows, err              = m.DB.Session(&gorm2.Session{}).Table(table).Limit(1).Rows()
		)
...
func (m Migrator) CurrentSchema(stmt *gorm2.Statement, table string) (string, string) {
...
func (m Migrator) TableType(value any) (tableType gorm2.TableType, err error) {
	var table migrator2.TableType
