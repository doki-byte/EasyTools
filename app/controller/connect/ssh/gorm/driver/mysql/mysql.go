...
func (dialector Dialector) ClauseBuilders() map[string]clause2.ClauseBuilder {
	clauseBuilders := map[string]clause2.ClauseBuilder{
		ClauseOnConflict: func(c clause2.Clause, builder clause2.Builder) {
			onConflict, ok := c.Expression.(clause2.OnConflict)
			if !ok {
				c.Build(builder)
				return
			}
			...
			if len(onConflict.DoUpdates) == 0 {
				if s := builder.(*gorm2.Statement).Schema; s != nil {
					var column clause2.Column
					onConflict.DoNothing = false

					if s.PrioritizedPrimaryField != nil {
						column = clause2.Column{Name: s.PrioritizedPrimaryField.DBName}
					} else if len(s.DBNames) > 0 {
						column = clause2.Column{Name: s.DBNames[0]}
					}

					if column.Name != "" {
						onConflict.DoUpdates = []clause2.Assignment{{Column: column, Value: column}}
					}

					builder.(*gorm2.Statement).AddClause(onConflict)
				}
			}
			...
			for idx, assignment := range onConflict.DoUpdates {
				...
				builder.WriteByte('=')
				if column, ok := assignment.Value.(clause2.Column); ok && column.Table == "excluded" {
					column.Table = ""
		...
		ClauseValues: func(c clause2.Clause, builder clause2.Builder) {
			if values, ok := c.Expression.(clause2.Values); ok && len(values.Columns) == 0 {
				builder.WriteString("VALUES()")
				return
			}
			c.Build(builder)
	...
	if dialector.Config.DontSupportForShareClause {
		clauseBuilders[ClauseFor] = func(c clause2.Clause, builder clause2.Builder) {
			if values, ok := c.Expression.(clause2.Locking); ok && strings.EqualFold(values.Strength, "SHARE") {
...
func (dialector Dialector) DefaultValueOf(field *schema.Field) clause2.Expression {
