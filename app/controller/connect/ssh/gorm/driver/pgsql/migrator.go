...
import (
	...
	migrator2 "EasyTools/app/controller/connect/ssh/gorm/migrator"
...
func (m Migrator) queryRaw(sql string, values ...any) (tx *gorm2.DB) {
	queryTx := m.DB
	if m.DB.DryRun {
		queryTx = m.DB.Session(&gorm2.Session{})
...
func (m Migrator) BuildIndexOptions(opts []schema2.IndexOption, stmt *gorm2.Statement) (results []any) {
	for _, opt := range opts {
		...
		results = append(results, clause2.Expr{SQL: str})
	}
	return
}
...
func (m Migrator) HasIndex(value any, name string) bool {
	var count int64
	m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		if stmt.Schema != nil {
...
func (m Migrator) CreateIndex(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		if stmt.Schema != nil {
			if idx := stmt.Schema.LookIndex(name); idx != nil {
				opts := m.BuildIndexOptions(idx.Fields, stmt)
				values := []any{clause2.Column{Name: idx.Name}, m.CurrentTable(stmt), opts}
...
func (m Migrator) RenameIndex(value any, oldName, newName string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		return m.DB.Exec(
			...
			clause2.Column{Name: oldName}, clause2.Column{Name: newName},
		).Error
	})
}
...
func (m Migrator) DropIndex(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		return m.DB.Exec("DROP INDEX ?", clause2.Column{Name: name}).Error
	})
}
...
func (m Migrator) CreateTable(values ...any) (err error) {
	...
	for _, value := range m.ReorderModels(values, false) {
		if err = m.RunWithValue(value, func(stmt *gorm2.Statement) error {
			if stmt.Schema != nil {
				for _, fieldName := range stmt.Schema.DBNames {
					...
					if field.Comment != "" {
						if err := m.DB.Exec(
							...
							m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, gorm2.Expr(m.Migrator.Dialector.Explain("$1", field.Comment)),
						).Error; err != nil {
							return err
						}
					}
...
func (m Migrator) DropTable(values ...any) error {
	...
	tx := m.DB.Session(&gorm2.Session{})
	for i := len(values) - 1; i >= 0; i-- {
		if err := m.RunWithValue(values[i], func(stmt *gorm2.Statement) error {
...
func (m Migrator) MigrateColumn(value any, field *schema2.Field, columnType gorm2.ColumnType) error {
	// skip primary field
	if !field.PrimaryKey {
...
func (m Migrator) AlterColumn(value any, field string) error {
	err := m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		if stmt.Schema != nil {
			if field := stmt.Schema.LookUpField(field); field != nil {
				var (
					columnTypes, _  = m.DB.Migrator().ColumnTypes(value)
					fieldColumnType *migrator2.ColumnType
				)
				for _, columnType := range columnTypes {
					if columnType.Name() == field.DBName {
						fieldColumnType, _ = columnType.(*migrator2.ColumnType)
					}
				}

				fileType := clause2.Expr{SQL: m.DataTypeOf(field)}
				...
				if null, _ := fieldColumnType.Nullable(); null == field.NotNull {
					if field.NotNull {
						if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? SET NOT NULL", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}).Error; err != nil {
							return err
						}
					} else {
						if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? DROP NOT NULL", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}).Error; err != nil {
							return err
						}
					}
				}
				...
				if v, ok := fieldColumnType.DefaultValue(); (field.DefaultValueInterface == nil && ok) || v != field.DefaultValue {
					if field.HasDefaultValue && (field.DefaultValueInterface != nil || field.DefaultValue != "") {
						if field.DefaultValueInterface != nil {
							defaultStmt := &gorm2.Statement{Vars: []any{field.DefaultValueInterface}}
							m.Dialector.BindVarTo(defaultStmt, defaultStmt, field.DefaultValueInterface)
							if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? SET DEFAULT ?", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, clause2.Expr{SQL: m.Dialector.Explain(defaultStmt.SQL.String(), field.DefaultValueInterface)}).Error; err != nil {
								return err
							}
						} else if field.DefaultValue != "(-)" {
							if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? SET DEFAULT ?", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, clause2.Expr{SQL: field.DefaultValue}).Error; err != nil {
								return err
							}
						} else {
							if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? DROP DEFAULT", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, clause2.Expr{SQL: field.DefaultValue}).Error; err != nil {
								return err
							}
						}
					}
...
func (m Migrator) modifyColumn(stmt *gorm2.Statement, field *schema2.Field, targetType clause2.Expr, existingColumn *migrator2.ColumnType) error {
	...
	if dv, _ := existingColumn.DefaultValue(); dv != "" && isUncastableDefaultValue {
		if err := m.DB.Exec("ALTER TABLE ? ALTER COLUMN ? DROP DEFAULT", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}).Error; err != nil {
			return err
		}
	}
	if err := m.DB.Exec(alterSQL, m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, targetType, clause2.Column{Name: field.DBName}, targetType).Error; err != nil {
		return err
	}
	return nil
}
...
func (m Migrator) ColumnTypes(value any) (columnTypes []gorm2.ColumnType, err error) {
	columnTypes = make([]gorm2.ColumnType, 0)
	err = m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		var (
			...
			for _, columnType := range columnTypes {
				for _, c := range rawColumnTypes {
					if c.Name() == columnType.Name() {
						columnType.(*migrator2.ColumnType).SQLColumnType = c
						break
					}
				}
			}
...
func (m Migrator) GetRows(currentSchema any, table any) (*sql.Rows, error) {
	...
	return m.DB.Session(&gorm2.Session{}).Table(name).Limit(1).Scopes(func(d *gorm2.DB) *gorm2.DB {
...
func (m Migrator) CurrentSchema(stmt *gorm2.Statement, table string) (any, any) {
	...
	return clause2.Expr{SQL: "CURRENT_SCHEMA()"}, table
}

func (m Migrator) CreateSequence(tx *gorm2.DB, stmt *gorm2.Statement, field *schema2.Field,
	...
	if err = tx.Exec(`CREATE SEQUENCE IF NOT EXISTS ? AS ?`, clause2.Expr{SQL: sequenceName},
		clause2.Expr{SQL: serialDatabaseType}).Error; err != nil {
		return err
	}

	if err := tx.Exec("ALTER TABLE ? ALTER COLUMN ? SET DEFAULT nextval('?')",
		clause2.Expr{SQL: tableName}, clause2.Expr{SQL: field.DBName}, clause2.Expr{SQL: sequenceName}).Error; err != nil {
		return err
	}

	if err := tx.Exec("ALTER SEQUENCE ? OWNED BY ?.?",
		clause2.Expr{SQL: sequenceName}, clause2.Expr{SQL: tableName}, clause2.Expr{SQL: field.DBName}).Error; err != nil {
		return err
	}
	return
}

func (m Migrator) UpdateSequence(tx *gorm2.DB, stmt *gorm2.Statement, field *schema2.Field,
	serialDatabaseType string) (err error) {
	...
	if err = tx.Exec(`ALTER SEQUENCE IF EXISTS ? AS ?`, clause2.Expr{SQL: sequenceName}, clause2.Expr{SQL: serialDatabaseType}).Error; err != nil {
		return err
	}

	if err := tx.Exec("ALTER TABLE ? ALTER COLUMN ? TYPE ?",
		m.CurrentTable(stmt), clause2.Expr{SQL: field.DBName}, clause2.Expr{SQL: serialDatabaseType}).Error; err != nil {
		return err
	}
	return
}

func (m Migrator) DeleteSequence(tx *gorm2.DB, stmt *gorm2.Statement, field *schema2.Field,
	fileType clause2.Expr) (err error) {
	...
	if err := tx.Exec("ALTER TABLE ? ALTER COLUMN ? TYPE ?", m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, fileType).Error; err != nil {
		return err
	}

	if err := tx.Exec("ALTER TABLE ? ALTER COLUMN ? DROP DEFAULT",
		m.CurrentTable(stmt), clause2.Expr{SQL: field.DBName}).Error; err != nil {
		return err
	}

	if err = tx.Exec(`DROP SEQUENCE IF EXISTS ?`, clause2.Expr{SQL: sequenceName}).Error; err != nil {
		return err
	}
...
func (m Migrator) getColumnSequenceName(tx *gorm2.DB, stmt *gorm2.Statement, field *schema2.Field) (
...
func (m Migrator) GetIndexes(value any) ([]gorm2.Index, error) {
	indexes := make([]gorm2.Index, 0)
...
func (m Migrator) resetPreparedStmts() {
	if m.DB.PrepareStmt {
		if pdb, ok := m.DB.ConnPool.(*gorm2.PreparedStmtDB); ok {
			pdb.Reset()
		}
	}
}
