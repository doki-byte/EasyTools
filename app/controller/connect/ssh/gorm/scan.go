...
func (db *DB) scanIntoStruct(rows Rows, reflectValue reflect.Value, values []any, fields []*schema2.Field, joinFields [][]*schema2.Field) {
...
func Scan(rows Rows, db *DB, mode ScanMode) {
	...
	default:
		var (
			fields       = make([]*schema2.Field, len(columns))
			joinFields   [][]*schema2.Field
		...
		if sch != nil {
			if reflectValueType != sch.ModelType && reflectValueType.Kind() == reflect.Struct {
				sch, _ = schema2.Parse(db.Statement.Dest, db.cacheStore, db.NamingStrategy)
			}
			...
			if len(columns) == 1 {
				...
				if _, ok := reflect.New(reflectValueType).Interface().(sql.Scanner); (reflectValueType != sch.ModelType && ok) || // is scanner
					...
					sch.ModelType.ConvertibleTo(schema2.TimeReflectType) { // is time
					sch = nil
				}
			}
				...
				for idx, column := range columns {
					if field := sch.LookUpField(column); field != nil && field.Readable {
						...
						if rel, ok := sch.Relationships.Relations[names[0]]; ok {
							...
							relFields := make([]*schema2.Field, 0, subNameCount-1)
							...
							if field := rel.FieldSchema.LookUpField(dbName); field != nil && field.Readable {
								...
								if len(joinFields) == 0 {
									joinFields = make([][]*schema2.Field, len(columns))
								}
