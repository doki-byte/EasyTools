...
import (
	...
	"EasyTools/app/controller/connect/ssh/gorm/inflection"
	"context"
	"fmt"
	"reflect"
	"strings"
...
func (constraint *Constraint) Build() (sql string, vars []any) {
	...
	for _, field := range constraint.ForeignKeys {
		foreignKeys = append(foreignKeys, clause2.Column{Name: field.DBName})
	}

	references := make([]any, 0, len(constraint.References))
	for _, field := range constraint.References {
		references = append(references, clause2.Column{Name: field.DBName})
	}
	vars = append(vars, clause2.Table{Name: constraint.Name}, foreignKeys, clause2.Table{Name: constraint.ReferenceSchema.Table}, references)
	return
}
...
func (rel *Relationship) ToQueryConditions(ctx context.Context, reflectValue reflect.Value) (conds []clause2.Expression) {
	...
	if rel.JoinTable != nil {
		...
		for _, ref := range rel.References {
			if ref.OwnPrimaryKey {
				...
				conds = append(conds, clause2.Eq{
					Column: clause2.Column{Table: rel.JoinTable.Table, Name: ref.ForeignKey.DBName},
					Value:  clause2.Column{Table: rel.FieldSchema.Table, Name: ref.PrimaryKey.DBName},
				})
			}
		}
	} else {
		for _, ref := range rel.References {
			if ref.OwnPrimaryKey {
				...
				conds = append(conds, clause2.Eq{
					Column: clause2.Column{Table: rel.FieldSchema.Table, Name: ref.ForeignKey.DBName},
					Value:  ref.PrimaryValue,
				})
			} else {
	...
	conds = append(conds, clause2.IN{Column: column, Values: values})
	return
}
