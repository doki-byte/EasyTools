...
func SetupUpdateReflectValue(db *gorm2.DB) {
...
func BeforeUpdate(db *gorm2.DB) {
	if db.Error == nil && db.Statement.Schema != nil && !db.Statement.SkipHooks && (db.Statement.Schema.BeforeSave || db.Statement.Schema.BeforeUpdate) {
		callMethod(db, func(value any, tx *gorm2.DB) (called bool) {
...
// Update update hook
func Update(config *Config) func(db *gorm2.DB) {
	...
	return func(db *gorm2.DB) {
		...
		if db.Statement.SQL.Len() == 0 {
			...
			db.Statement.AddClauseIfNotExists(clause2.Update{})
...
func ConvertToAssignments(stmt *gorm2.Statement) (set clause2.Set) {
	var (
		selectColumns, restricted = stmt.SelectAndOmitColumns(false, true)
		assignValue               func(field *schema2.Field, value any)
	)
	...
	if !updatingValue.CanAddr() || stmt.Dest != stmt.Model {
		...
		case reflect.Slice, reflect.Array:
			if size := stmt.ReflectValue.Len(); size > 0 {
				...
				if !isZero {
					_, primaryValues := schema2.GetIdentityFieldValuesMap(stmt.Context, stmt.ReflectValue, stmt.Schema.PrimaryFields)
					column, values := schema2.ToQueryValues("", stmt.Schema.PrimaryFieldDBNames, primaryValues)
		...
		case reflect.Struct:
			for _, field := range stmt.Schema.PrimaryFields {
				if value, isZero := field.ValueOf(stmt.Context, stmt.ReflectValue); !isZero {
					stmt.AddClause(clause2.Where{Exprs: []clause2.Expression{clause2.Eq{Column: field.DBName, Value: value}}})
				}
			}
		}
	}
	...
	case map[string]any:
		set = make([]clause2.Assignment, 0, len(value))
		...
		for _, k := range keys {
				...
				if field := stmt.Schema.LookUpField(k); field != nil {
					if field.DBName != "" {
						if v, ok := selectColumns[field.DBName]; (ok && v) || (!ok && !restricted) {
							set = append(set, clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: kv})
			...
			if v, ok := selectColumns[k]; (ok && v) || (!ok && !restricted) {
				set = append(set, clause2.Assignment{Column: clause2.Column{Name: k}, Value: kv})
			}
		}
			...
			for _, dbName := range stmt.Schema.DBNames {
				...
				if field.AutoUpdateTime > 0 && value[field.Name] == nil && value[field.DBName] == nil {
					if v, ok := selectColumns[field.DBName]; (ok && v) || !ok {
						...
						if field.AutoUpdateTime == schema2.UnixNanosecond {
							set = append(set, clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: now.UnixNano()})
						} else if field.AutoUpdateTime == schema2.UnixMillisecond {
							set = append(set, clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: now.UnixMilli()})
						} else if field.AutoUpdateTime == schema2.UnixSecond {
							set = append(set, clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: now.Unix()})
						} else {
							set = append(set, clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: now})
						}
					}
				}
			}
	...
	default:
		...
		if !updatingValue.CanAddr() || stmt.Dest != stmt.Model {
			// different schema
			updatingStmt := &gorm2.Statement{DB: stmt.DB}
		...
		case reflect.Struct:
			set = make([]clause2.Assignment, 0, len(stmt.Schema.FieldsByDBName))
			for _, dbName := range stmt.Schema.DBNames {
					...
					if !field.PrimaryKey || !updatingValue.CanAddr() || stmt.Dest != stmt.Model {
						if v, ok := selectColumns[field.DBName]; (ok && v) || (!ok && (!restricted || (!stmt.SkipHooks && field.AutoUpdateTime > 0))) {
							...
							if (ok || !isZero) && field.Updatable {
								set = append(set, clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: value})
								assignField := field
								if isDiffSchema {
		...
		default:
			stmt.AddError(gorm2.ErrInvalidData)
		}
	}
