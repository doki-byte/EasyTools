...
//		},
//	}
func parsePreloadMap(s *schema2.Schema, preloads map[string][]any) map[string]map[string][]any {
	...
	for name, args := range preloads {
		...
		if preloadFields[0] == clause2.Associations {
...
func embeddedValues(embeddedRelations *schema2.Relationships) []string {
...
//
//nolint:cyclop
func preloadEntryPoint(db *gorm2.DB, joins []string, relationships *schema2.Relationships, preloads map[string][]any, associationsConds []any) error {
	...
	for _, name := range preloadNames {
		if relations := relationships.EmbeddedRelations[name]; relations != nil {
			...
			if joined, nestedJoins := isJoined(name); joined {
				...
				default:
					return gorm2.ErrInvalidData
				}
			} else {
				tx := db.Table("").Session(&gorm2.Session{Context: db.Statement.Context, SkipHooks: db.Statement.SkipHooks})
			...
			return fmt.Errorf("%s: %w for schema %s", name, gorm2.ErrUnsupportedRelation, db.Statement.Schema.Name)
		}
	}
	return nil
}

func preloadDB(db *gorm2.DB, reflectValue reflect.Value, dest any) *gorm2.DB {
	tx := db.Session(&gorm2.Session{Context: db.Statement.Context, NewDB: true, SkipHooks: db.Statement.SkipHooks, Initialized: true})
...
func preload(tx *gorm2.DB, rel *schema2.Relationship, conds []any, preloads map[string][]any) error {
	var (
		reflectValue     = tx.Statement.ReflectValue
		relForeignKeys   []string
		relForeignFields []*schema2.Field
		foreignFields    []*schema2.Field
		foreignValues    [][]any
	...
	if rel.JoinTable != nil {
		var (
			joinForeignFields    = make([]*schema2.Field, 0, len(rel.References))
			joinRelForeignFields = make([]*schema2.Field, 0, len(rel.References))
		...
		for _, ref := range rel.References {
			if ref.OwnPrimaryKey {
				...
				tx = tx.Where(clause2.Eq{Column: ref.ForeignKey.DBName, Value: ref.PrimaryValue})
			} else {
		...
		joinIdentityMap, joinForeignValues := schema2.GetIdentityFieldValuesMap(tx.Statement.Context, reflectValue, foreignFields)
		if len(joinForeignValues) == 0 {
			return nil
		}

		joinResults := rel.JoinTable.MakeSlice().Elem()
		column, values := schema2.ToQueryValues(clause2.CurrentTable, joinForeignKeys, joinForeignValues)
		if err := tx.Where(clause2.IN{Column: column, Values: values}).Find(joinResults.Addr().Interface()).Error; err != nil {
			return err
		}
		...
		_, foreignValues = schema2.GetIdentityFieldValuesMap(tx.Statement.Context, joinResults, joinRelForeignFields)
	} else {
		for _, ref := range rel.References {
			if ref.OwnPrimaryKey {
		...
		identityMap, foreignValues = schema2.GetIdentityFieldValuesMap(tx.Statement.Context, reflectValue, foreignFields)
	...
	column, values := schema2.ToQueryValues(clause2.CurrentTable, relForeignKeys, foreignValues)

	if len(values) != 0 {
		for _, cond := range conds {
			if fc, ok := cond.(func(*gorm2.DB) *gorm2.DB); ok {
				tx = fc(tx)
			} else {
		...
		if err := tx.Where(clause2.IN{Column: column, Values: values}).Find(reflectResults.Addr().Interface(), inlineConds...).Error; err != nil {
			return err
		}
	}
