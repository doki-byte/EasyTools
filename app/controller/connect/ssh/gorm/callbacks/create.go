...
// Create create hook
func Create(config *Config) func(db *gorm2.DB) {
	...
	return func(db *gorm2.DB) {
		...
		if db.Statement.Schema != nil {
			...
			if supportReturning && len(db.Statement.Schema.FieldsWithDefaultDBValue) > 0 {
				if _, ok := db.Statement.Clauses["RETURNING"]; !ok {
					fromColumns := make([]clause2.Column, 0, len(db.Statement.Schema.FieldsWithDefaultDBValue))
					for _, field := range db.Statement.Schema.FieldsWithDefaultDBValue {
						fromColumns = append(fromColumns, clause2.Column{Name: field.DBName})
					}
					db.Statement.AddClause(clause2.Returning{Columns: fromColumns})
				}
			}
		}
		...
		if db.Statement.SQL.Len() == 0 {
			...
			db.Statement.AddClauseIfNotExists(clause2.Insert{})
		...
		if ok {
			if c, ok := db.Statement.Clauses["ON CONFLICT"]; ok {
				if onConflict, _ := c.Expression.(clause2.OnConflict); onConflict.DoNothing {
					mode |= gorm2.ScanOnConflictDoNothing
				}
			}
...
func ConvertToCreateValues(stmt *gorm2.Statement) (values clause2.Values) {
	...
	default:
		...
		values = clause2.Values{Columns: make([]clause2.Column, 0, len(stmt.Schema.DBNames))}

		for _, db := range stmt.Schema.DBNames {
			if field := stmt.Schema.FieldsByDBName[db]; !field.HasDefaultValue || field.DefaultValueInterface != nil {
				if v, ok := selectColumns[db]; (ok && v) || (!ok && (!restricted || field.AutoCreateTime > 0 || field.AutoUpdateTime > 0)) {
					values.Columns = append(values.Columns, clause2.Column{Name: db})
				}
			}
		}
		...
		case reflect.Slice, reflect.Array:
			...
			for i := 0; i < rValLen; i++ {
				...
				if !rv.IsValid() {
					stmt.AddError(fmt.Errorf("slice data #%v is invalid: %w", i, gorm2.ErrInvalidData))
					return
				}
			...
			for _, field := range stmt.Schema.FieldsWithDefaultDBValue {
				if vs, ok := defaultValueFieldsHavingValue[field]; ok {
					values.Columns = append(values.Columns, clause2.Column{Name: field.DBName})
	...
	if c, ok := stmt.Clauses["ON CONFLICT"]; ok {
		if onConflict, _ := c.Expression.(clause2.OnConflict); onConflict.UpdateAll {
			if stmt.Schema != nil && len(values.Columns) >= 1 {
						...
						if v, ok := selectColumns[field.DBName]; (ok && v) || (!ok && !restricted) {
							if !field.PrimaryKey && (!field.HasDefaultValue || field.DefaultValueInterface != nil ||
								...
								if field.AutoUpdateTime > 0 {
									assignment := clause2.Assignment{Column: clause2.Column{Name: field.DBName}, Value: curTime}
				...
				onConflict.DoUpdates = append(onConflict.DoUpdates, clause2.AssignmentColumns(columns)...)
				...
				if len(onConflict.Columns) == 0 {
					for _, field := range stmt.Schema.PrimaryFields {
						onConflict.Columns = append(onConflict.Columns, clause2.Column{Name: field.DBName})
					}
				}
