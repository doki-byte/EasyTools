...
func SaveBeforeAssociations(create bool) func(db *gorm2.DB) {
...
func SaveAfterAssociations(create bool) func(db *gorm2.DB) {
	return func(db *gorm2.DB) {
		if db.Error == nil && db.Statement.Schema != nil {
			selectColumns, restricted := db.Statement.SelectAndOmitColumns(create, !create)
			...
			for _, rel := range db.Statement.Schema.Relationships.Many2Many {
				...
				if joins.Len() > 0 {
					db.AddError(db.Session(&gorm2.Session{NewDB: true}).Clauses(clause2.OnConflict{DoNothing: true}).Session(&gorm2.Session{
...
func onConflictOption(stmt *gorm2.Statement, s *schema2.Schema, defaultUpdatingColumns []string) (onConflict clause2.OnConflict) {
	if len(defaultUpdatingColumns) > 0 || stmt.DB.FullSaveAssociations {
		onConflict.Columns = make([]clause2.Column, 0, len(s.PrimaryFieldDBNames))
		for _, dbName := range s.PrimaryFieldDBNames {
			onConflict.Columns = append(onConflict.Columns, clause2.Column{Name: dbName})
		}

		onConflict.UpdateAll = stmt.DB.FullSaveAssociations
		if !onConflict.UpdateAll {
			onConflict.DoUpdates = clause2.AssignmentColumns(defaultUpdatingColumns)
		}
	} else {
...
func saveAssociations(db *gorm2.DB, rel *schema2.Relationship, rValues reflect.Value, selectColumns map[string]bool, restricted bool, defaultUpdatingColumns []string) error {
	...
	tx := db.Session(&gorm2.Session{NewDB: true}).Clauses(onConflict).Session(&gorm2.Session{
	...
	if len(selects) > 0 {
		...
		tx = tx.Omit(clause2.Associations)
	}
...
func checkAssociationsSaved(db *gorm2.DB, values reflect.Value) bool {
