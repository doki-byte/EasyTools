...
var _ gorm2.Migrator = (*Migrator)(nil)
...
type GormDataTypeInterface interface {
	GormDBDataType(*gorm2.DB, *schema2.Field) string
}

// RunWithValue run migration with statement value
func (m Migrator) RunWithValue(value any, fc func(*gorm2.Statement) error) error {
	stmt := &gorm2.Statement{DB: m.DB}
...
func (m Migrator) DataTypeOf(field *schema2.Field) string {
...
func (m Migrator) FullDataTypeOf(field *schema2.Field) (expr clause2.Expr) {
...
func (m Migrator) GetQueryAndExecTx() (queryTx, execTx *gorm2.DB) {
	queryTx = m.DB.Session(&gorm2.Session{})
	execTx = queryTx
	if m.DB.DryRun {
		queryTx.DryRun = false
		execTx = m.DB.Session(&gorm2.Session{Logger: &printSQLLogger{Interface: m.DB.Logger}})
	}
		...
		if !queryTx.Migrator().HasTable(value) {
			...
			if err := m.RunWithValue(value, func(stmt *gorm2.Statement) error {
				...
				for _, dbName := range stmt.Schema.DBNames {
					var foundColumn gorm2.ColumnType
...
func (m Migrator) CreateTable(values ...any) error {
	for _, value := range m.ReorderModels(values, false) {
		...
		if err := m.RunWithValue(value, func(stmt *gorm2.Statement) (err error) {
			var (
			...
			for _, dbName := range stmt.Schema.DBNames {
				...
				if !field.IgnoreMigration {
					...
					values = append(values, clause2.Column{Name: dbName}, m.DB.Migrator().FullDataTypeOf(field))
					createTableSQL += ","
				}
			}
			...
			if !hasPrimaryKeyInDataType && len(stmt.Schema.PrimaryFields) > 0 {
				...
				for _, field := range stmt.Schema.PrimaryFields {
					primaryKeys = append(primaryKeys, clause2.Column{Name: field.DBName})
				}
			...
			for _, idx := range stmt.Schema.ParseIndexes() {
				if m.CreateIndexAfterCreateTable {
					...
					createTableSQL += ","
					values = append(values, clause2.Column{Name: idx.Name}, tx.Migrator().(BuildIndexOptionsInterface).BuildIndexOptions(idx.Fields, stmt))
				}
			}
			...
			for _, uni := range stmt.Schema.ParseUniqueConstraints() {
				...
				values = append(values, clause2.Column{Name: uni.Name}, clause2.Expr{SQL: stmt.Quote(uni.Field.DBName)})
			}

			for _, chk := range stmt.Schema.ParseCheckConstraints() {
				createTableSQL += "CONSTRAINT ? CHECK (?),"
				values = append(values, clause2.Column{Name: chk.Name}, clause2.Expr{SQL: chk.Constraint})
			}
...
func (m Migrator) AddColumn(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		if !f.IgnoreMigration {
			return m.DB.Exec(
				"ALTER TABLE ? ADD ? ?",
				m.CurrentTable(stmt), clause2.Column{Name: f.DBName}, m.DB.Migrator().FullDataTypeOf(f),
			).Error
		}
...
func (m Migrator) DropColumn(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		return m.DB.Exec(
			"ALTER TABLE ? DROP COLUMN ?", m.CurrentTable(stmt), clause2.Column{Name: name},
		).Error
	})
}
	...
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		if field := stmt.Schema.LookUpField(field); field != nil {
			...
			return m.DB.Exec(
				...
				m.CurrentTable(stmt), clause2.Column{Name: field.DBName}, fileType,
			).Error
...
func (m Migrator) RenameColumn(value any, oldName, newName string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		return m.DB.Exec(
			...
			m.CurrentTable(stmt), clause2.Column{Name: oldName}, clause2.Column{Name: newName},
		).Error
	})
}
...
func (m Migrator) MigrateColumnUnique(value any, field *schema2.Field, columnType gorm2.ColumnType) error {
...
func (m Migrator) ColumnTypes(value any) ([]gorm2.ColumnType, error) {
	columnTypes := make([]gorm2.ColumnType, 0)
	execErr := m.RunWithValue(value, func(stmt *gorm2.Statement) (err error) {
		rows, err := m.DB.Session(&gorm2.Session{}).Table(stmt.Table).Limit(1).Rows()
		if err != nil {
			return err
		}
...
func (m Migrator) CreateView(name string, option gorm2.ViewOption) error {
	if option.Query == nil {
		return gorm2.ErrSubQueryRequired
	}
...
func (m Migrator) DropView(name string) error {
	return m.DB.Exec("DROP VIEW IF EXISTS ?", clause2.Table{Name: name}).Error
}

// GuessConstraintAndTable guess statement's constraint and it's table based on name
//
// Deprecated: use GuessConstraintInterfaceAndTable instead.
func (m Migrator) GuessConstraintAndTable(stmt *gorm2.Statement, name string) (*schema2.Constraint, *schema2.CheckConstraint, string) {
	constraint, table := m.GuessConstraintInterfaceAndTable(stmt, name)
	switch c := constraint.(type) {
	case *schema2.Constraint:
		return c, nil, table
	case *schema2.CheckConstraint:
		return nil, c, table
	default:
...
// nolint:cyclop
func (m Migrator) GuessConstraintInterfaceAndTable(stmt *gorm2.Statement, name string) (_ schema2.ConstraintInterface, table string) {
	if stmt.Schema == nil {
	...
	getTable := func(rel *schema2.Relationship) string {
		switch rel.Type {
...
func (m Migrator) CreateConstraint(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		constraint, table := m.GuessConstraintInterfaceAndTable(stmt, name)
		if constraint != nil {
			vars := []any{clause2.Table{Name: table}}
...
func (m Migrator) DropConstraint(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		return m.DB.Exec("ALTER TABLE ? DROP CONSTRAINT ?", clause2.Table{Name: table}, clause2.Column{Name: name}).Error
	})
}
...
type BuildIndexOptionsInterface interface {
	BuildIndexOptions([]schema2.IndexOption, *gorm2.Statement) []any
}
...
func (m Migrator) DropIndex(value any, name string) error {
	return m.RunWithValue(value, func(stmt *gorm2.Statement) error {
		...
		return m.DB.Exec("DROP INDEX ? ON ?", clause2.Column{Name: name}, m.CurrentTable(stmt)).Error
	})
}
...
func (m Migrator) ReorderModels(values []any, autoAdd bool) (results []any) {
	...
	parseDependence = func(value any, addToList bool) {
		dep := Dependency{
			Statement: &gorm2.Statement{DB: m.DB, Dest: value},
		}
		beDependedOn := map[*schema2.Schema]bool{}
		...
		if !m.DB.IgnoreRelationshipsWhenMigrating {
			for _, rel := range dep.Schema.Relationships.Relations {
				...
				if rel.Type == schema2.HasOne || rel.Type == schema2.HasMany {
					beDependedOn[rel.FieldSchema] = true
				}

				if rel.JoinTable != nil {
					// append join value
					defer func(rel *schema2.Relationship, joinValue any) {
...
func (m Migrator) CurrentTable(stmt *gorm2.Statement) any {
	if stmt.TableExpr != nil {
		return *stmt.TableExpr
	}
	return clause2.Table{Name: stmt.Table}
}

// GetIndexes return Indexes []gorm.Index and execErr error
func (m Migrator) GetIndexes(dst any) ([]gorm2.Index, error) {
...
func (m Migrator) TableType(dst any) (gorm2.TableType, error) {
	return nil, errors.New("not support")
}
