...
type Association struct {
	DB           *DB
	Relationship *schema2.Relationship
	Unscope      bool
	Error        error
}
...
func (association *Association) Append(values ...any) error {
	if association.Error == nil {
		...
		case schema2.HasOne, schema2.BelongsTo:
			if len(values) > 0 {
...
func (association *Association) Replace(values ...any) error {
	if association.Error == nil {
		...
		var oldBelongsToExpr clause2.Expression
		// we have to record the old BelongsTo value
		if association.Unscope && rel.Type == schema2.BelongsTo {
			var foreignFields []*schema2.Field
			for _, ref := range rel.References {
				if !ref.OwnPrimaryKey {
					foreignFields = append(foreignFields, ref.ForeignKey)
				}
			}
			if _, fvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, foreignFields); len(fvs) > 0 {
				column, values := schema2.ToQueryValues(rel.FieldSchema.Table, rel.FieldSchema.PrimaryFieldDBNames, fvs)
				oldBelongsToExpr = clause2.IN{Column: column, Values: values}
			}
		}
		...
		case schema2.HasOne, schema2.HasMany:
			var (
				primaryFields []*schema2.Field
				foreignKeys   []string
				updateMap     = map[string]any{}
				relValues     = schema2.GetRelationsValues(association.DB.Statement.Context, reflectValue, []*schema2.Relationship{rel})
				modelValue    = reflect.New(rel.FieldSchema.ModelType).Interface()
				tx            = association.DB.Model(modelValue)
			)

			if _, rvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, relValues, rel.FieldSchema.PrimaryFields); len(rvs) > 0 {
				if column, values := schema2.ToQueryValues(rel.FieldSchema.Table, rel.FieldSchema.PrimaryFieldDBNames, rvs); len(values) > 0 {
					tx.Not(clause2.IN{Column: column, Values: values})
				}
			}
			...
			for _, ref := range rel.References {
				if ref.OwnPrimaryKey {
					...
					tx.Where(clause2.Eq{Column: ref.ForeignKey.DBName, Value: ref.PrimaryValue})
				}
			}

			if _, pvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, primaryFields); len(pvs) > 0 {
				column, values := schema2.ToQueryValues(rel.FieldSchema.Table, foreignKeys, pvs)
				if association.Unscope {
					association.Error = tx.Where(clause2.IN{Column: column, Values: values}).Delete(modelValue).Error
				} else {
					association.Error = tx.Where(clause2.IN{Column: column, Values: values}).UpdateColumns(updateMap).Error
				}
			}
		case schema2.Many2Many:
			var (
				primaryFields, relPrimaryFields     []*schema2.Field
			...
			for _, ref := range rel.References {
				if ref.PrimaryValue == "" {
					...
					tx.Clauses(clause2.Eq{Column: ref.ForeignKey.DBName, Value: ref.PrimaryValue})
				}
			}

			_, pvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, primaryFields)
			if column, values := schema2.ToQueryValues(rel.JoinTable.Table, joinPrimaryKeys, pvs); len(values) > 0 {
				tx.Where(clause2.IN{Column: column, Values: values})
			} else {
				return ErrPrimaryKeyRequired
			}

			_, rvs := schema2.GetIdentityFieldValuesMapFromValues(association.DB.Statement.Context, values, relPrimaryFields)
			if relColumn, relValues := schema2.ToQueryValues(rel.JoinTable.Table, joinRelPrimaryKeys, rvs); len(relValues) > 0 {
				tx.Where(clause2.Not(clause2.IN{Column: relColumn, Values: relValues}))
			}
	...
	if association.Error == nil {
		...
		for _, ref := range rel.References {
			if ref.PrimaryValue == "" {
				...
				conds = append(conds, clause2.Eq{Column: ref.ForeignKey.DBName, Value: ref.PrimaryValue})
			}
		}
		...
		case schema2.BelongsTo:
			...
			_, pvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, rel.Schema.PrimaryFields)
			if pcolumn, pvalues := schema2.ToQueryValues(rel.Schema.Table, rel.Schema.PrimaryFieldDBNames, pvs); len(pvalues) > 0 {
				conds = append(conds, clause2.IN{Column: pcolumn, Values: pvalues})
			} else {
				return ErrPrimaryKeyRequired
			}

			_, rvs := schema2.GetIdentityFieldValuesMapFromValues(association.DB.Statement.Context, values, primaryFields)
			relColumn, relValues := schema2.ToQueryValues(rel.Schema.Table, foreignKeys, rvs)
			conds = append(conds, clause2.IN{Column: relColumn, Values: relValues})
			...
			if association.Unscope {
				...
				if _, fvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, foreignFields); len(fvs) > 0 {
					column, values := schema2.ToQueryValues(rel.FieldSchema.Table, rel.FieldSchema.PrimaryFieldDBNames, fvs)
					association.Error = associationDB.Model(nil).Where(clause2.IN{Column: column, Values: values}).Delete(reflect.New(rel.FieldSchema.ModelType).Interface()).Error
				}
			}
		case schema2.HasOne, schema2.HasMany:
			...
			_, pvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, primaryFields)
			if pcolumn, pvalues := schema2.ToQueryValues(rel.FieldSchema.Table, foreignKeys, pvs); len(pvalues) > 0 {
				conds = append(conds, clause2.IN{Column: pcolumn, Values: pvalues})
			} else {
				return ErrPrimaryKeyRequired
			}

			_, rvs := schema2.GetIdentityFieldValuesMapFromValues(association.DB.Statement.Context, values, rel.FieldSchema.PrimaryFields)
			relColumn, relValues := schema2.ToQueryValues(rel.FieldSchema.Table, rel.FieldSchema.PrimaryFieldDBNames, rvs)
			conds = append(conds, clause2.IN{Column: relColumn, Values: relValues})
		...
		case schema2.Many2Many:
			...
			_, pvs := schema2.GetIdentityFieldValuesMap(association.DB.Statement.Context, reflectValue, primaryFields)
			if pcolumn, pvalues := schema2.ToQueryValues(rel.JoinTable.Table, joinPrimaryKeys, pvs); len(pvalues) > 0 {
				conds = append(conds, clause2.IN{Column: pcolumn, Values: pvalues})
			} else {
				return ErrPrimaryKeyRequired
			}
			...
			_, rvs := schema2.GetIdentityFieldValuesMapFromValues(association.DB.Statement.Context, values, relPrimaryFields)
			relColumn, relValues := schema2.ToQueryValues(rel.JoinTable.Table, joinRelPrimaryKeys, rvs)
			conds = append(conds, clause2.IN{Column: relColumn, Values: relValues})

			association.Error = association.DB.Where(clause2.Where{Exprs: conds}).Model(nil).Delete(joinValue).Error
		}

		if association.Error == nil {
			// clean up deleted values's foreign key
			relValuesMap, _ := schema2.GetIdentityFieldValuesMapFromValues(association.DB.Statement.Context, values, rel.FieldSchema.PrimaryFields)
...
func (association *Association) saveAssociation(clear bool, values ...any) {
	...
	appendToRelations := func(source, rv reflect.Value, clear bool) {
		...
		case schema2.HasMany, schema2.Many2Many:
	...
	for name, ok := range selectColumns {
		...
		} else if strings.HasPrefix(name, clause2.Associations) {
			columnName = name
		}
...
func (association *Association) buildCondition() *DB {
	...
	if association.Relationship.JoinTable != nil {
		if !tx.Statement.Unscoped && len(association.Relationship.JoinTable.QueryClauses) > 0 {
			joinStmt := Statement{DB: tx, Context: tx.Statement.Context, Schema: association.Relationship.JoinTable, Table: association.Relationship.JoinTable.Table, Clauses: map[string]clause2.Clause{}}
			...
			if len(joinStmt.SQL.String()) > 0 {
				tx.Clauses(clause2.Expr{SQL: strings.Replace(joinStmt.SQL.String(), "WHERE ", "", 1), Vars: joinStmt.Vars})
			}
		}

		tx = tx.Session(&Session{QueryFields: true}).Clauses(clause2.From{Joins: []clause2.Join{{
			Table: clause2.Table{Name: association.Relationship.JoinTable.Table},
			ON:    clause2.Where{Exprs: queryConds},
		}}})
	} else {
		tx.Clauses(clause2.Where{Exprs: queryConds})
	}
