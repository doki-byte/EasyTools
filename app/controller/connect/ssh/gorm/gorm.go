...
func Open(dialector Dialector, opts ...Option) (db *DB, err error) {
	...
	if config.NamingStrategy == nil {
		config.NamingStrategy = schema2.NamingStrategy{IdentifierMaxLength: 64} // Default Identifier length is 64
	}
	...
	if config.ClauseBuilders == nil {
		config.ClauseBuilders = map[string]clause2.ClauseBuilder{}
	}
...
func Expr(expr string, args ...any) clause2.Expr {
	return clause2.Expr{SQL: expr, Vars: args}
}
...
func (db *DB) SetupJoinTable(model any, field string, joinTable any) error {
	var (
		tx                      = db.getInstance()
		stmt                    = tx.Statement
		modelSchema, joinSchema *schema2.Schema
	)
