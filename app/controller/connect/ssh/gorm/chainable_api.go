...
func (db *DB) Clauses(conds ...clause2.Expression) (tx *DB) {
	tx = db.getInstance()
	var whereConds []any

	for _, cond := range conds {
		if c, ok := cond.(clause2.Interface); ok {
	...
	if len(whereConds) > 0 {
		tx.Statement.AddClause(clause2.Where{Exprs: tx.Statement.BuildCondition(whereConds[0], whereConds[1:]...)})
	}
	return
}
...
func (db *DB) Table(name string, args ...any) (tx *DB) {
	...
	if strings.Contains(name, " ") || strings.Contains(name, "`") || len(args) > 0 {
		tx.Statement.TableExpr = &clause2.Expr{SQL: name, Vars: args}
		...
		tx.Statement.TableExpr = &clause2.Expr{SQL: tx.Statement.Quote(name)}
...
func (db *DB) Select(query any, args ...any) (tx *DB) {
	...
	case string:
		if strings.Count(v, "?") >= len(args) && len(args) > 0 {
			tx.Statement.AddClause(clause2.Select{
				Distinct:   db.Statement.Distinct,
				Expression: clause2.Expr{SQL: v, Vars: args},
			})
		} else if strings.Count(v, "@") > 0 && len(args) > 0 {
			tx.Statement.AddClause(clause2.Select{
				Distinct:   db.Statement.Distinct,
				Expression: clause2.NamedExpr{SQL: v, Vars: args},
			})
		} else {
...
func (db *DB) Where(query any, args ...any) (tx *DB) {
	...
	if conds := tx.Statement.BuildCondition(query, args...); len(conds) > 0 {
		tx.Statement.AddClause(clause2.Where{Exprs: conds})
	}
	return
}
...
func (db *DB) Not(query any, args ...any) (tx *DB) {
	tx = db.getInstance()
	if conds := tx.Statement.BuildCondition(query, args...); len(conds) > 0 {
		tx.Statement.AddClause(clause2.Where{Exprs: []clause2.Expression{clause2.Not(conds...)}})
	}
	return
}
...
func (db *DB) Or(query any, args ...any) (tx *DB) {
	tx = db.getInstance()
	if conds := tx.Statement.BuildCondition(query, args...); len(conds) > 0 {
		tx.Statement.AddClause(clause2.Where{Exprs: []clause2.Expression{clause2.Or(clause2.And(conds...))}})
	}
	return
}
...
func (db *DB) Joins(query string, args ...any) (tx *DB) {
	return joins(db, clause2.LeftJoin, query, args...)
}
...
func (db *DB) InnerJoins(query string, args ...any) (tx *DB) {
	return joins(db, clause2.InnerJoin, query, args...)
}

func joins(db *DB, joinType clause2.JoinType, query string, args ...any) (tx *DB) {
	tx = db.getInstance()
	...
	if len(args) == 1 {
		if db, ok := args[0].(*DB); ok {
			...
			if where, ok := db.Statement.Clauses["WHERE"].Expression.(clause2.Where); ok {
				j.On = &where
			}
...
func (db *DB) Group(name string) (tx *DB) {
	...
	tx.Statement.AddClause(clause2.GroupBy{
		Columns: []clause2.Column{{Name: name, Raw: len(fields) != 1}},
	})
	return
}
...
func (db *DB) Order(value any) (tx *DB) {
	...
	switch v := value.(type) {
	case clause2.OrderByColumn:
		tx.Statement.AddClause(clause2.OrderBy{
			Columns: []clause2.OrderByColumn{v},
		})
	case string:
		if v != "" {
			tx.Statement.AddClause(clause2.OrderBy{
				Columns: []clause2.OrderByColumn{{
					Column: clause2.Column{Name: v, Raw: true},
				}},
			})
		}
	}
...
func (db *DB) Limit(limit int) (tx *DB) {
	tx = db.getInstance()
	tx.Statement.AddClause(clause2.Limit{Limit: &limit})
	return
}
...
func (db *DB) Offset(offset int) (tx *DB) {
	tx = db.getInstance()
	tx.Statement.AddClause(clause2.Limit{Offset: offset})
	return
}
