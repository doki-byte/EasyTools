...
func (d *decoder) expr() *unstable2.Node {
...
func (d *decoder) handleRootExpression(expr *unstable2.Node, v reflect.Value) error {
	var x reflect.Value
	var err error
	var first bool // used for to clear array tables on first use

	if !(d.skipUntilTable && expr.Kind == unstable2.KeyValue) {
	...
	if d.skipUntilTable {
		if expr.Kind == unstable2.Table || expr.Kind == unstable2.ArrayTable {
...
func (d *decoder) handleArrayTable(key unstable2.Iterator, v reflect.Value) (reflect.Value, error) {
	if key.Next() {
...
func (d *decoder) handleArrayTableCollectionLast(key unstable2.Iterator, v reflect.Value) (reflect.Value, error) {
	switch v.Kind() {
...
func (d *decoder) handleArrayTableCollection(key unstable2.Iterator, v reflect.Value) (reflect.Value, error) {
	if key.IsLast() {
...
func (d *decoder) handleKeyPart(key unstable2.Iterator, v reflect.Value, nextFn handlerFn, makeFn valueMakerFn) (reflect.Value, error) {
...
func (d *decoder) handleArrayTablePart(key unstable2.Iterator, v reflect.Value) (reflect.Value, error) {
...
// cannot handle it.
func (d *decoder) handleTable(key unstable2.Iterator, v reflect.Value) (reflect.Value, error) {
	if v.Kind() == reflect.Slice {
		if v.Len() == 0 {
			return reflect.Value{}, unstable2.NewParserError(key.Node().Data, "cannot store a table in a slice")
		}
		elem := v.Index(v.Len() - 1)
...
func (d *decoder) handleKeyValues(v reflect.Value) (reflect.Value, error) {
	...
	for d.nextExpr() {
		expr := d.expr()
		if expr.Kind != unstable2.KeyValue {
...
type (
	handlerFn    func(key unstable2.Iterator, v reflect.Value) (reflect.Value, error)
...
func (d *decoder) handleTablePart(key unstable2.Iterator, v reflect.Value) (reflect.Value, error) {
	return d.handleKeyPart(key, v, d.handleTable, makeMapStringInterface)
}

func (d *decoder) tryTextUnmarshaler(node *unstable2.Node, v reflect.Value) (bool, error) {
	...
	if v.CanAddr() && v.Addr().Type().Implements(textUnmarshalerType) {
		...
		if err != nil {
			return false, unstable2.NewParserError(d.p.Raw(node.Raw), "%w", err)
		}
...
func (d *decoder) handleValue(value *unstable2.Node, v reflect.Value) error {
	...
	if d.unmarshalerInterface {
		if v.CanAddr() && v.Addr().CanInterface() {
			if outi, ok := v.Addr().Interface().(unstable2.Unmarshaler); ok {
...
func (d *decoder) unmarshalArray(array *unstable2.Node, v reflect.Value) error {
	switch v.Kind() {
	case reflect.Slice:
		if v.IsNil() {
...
func (d *decoder) unmarshalInlineTable(itable *unstable2.Node, v reflect.Value) error {
	...
	default:
		return unstable2.NewParserError(d.p.Raw(itable.Raw), "cannot store inline table in Go type %s", v.Kind())
	}
...
func (d *decoder) unmarshalDateTime(value *unstable2.Node, v reflect.Value) error {
...
func (d *decoder) unmarshalLocalDate(value *unstable2.Node, v reflect.Value) error {
...
func (d *decoder) unmarshalLocalTime(value *unstable2.Node, v reflect.Value) error {
	...
	if len(rest) > 0 {
		return unstable2.NewParserError(rest, "extra characters at the end of a local time")
	}
...
func (d *decoder) unmarshalLocalDateTime(value *unstable2.Node, v reflect.Value) error {
	...
	if len(rest) > 0 {
		return unstable2.NewParserError(rest, "extra characters at the end of a local date time")
	}
...
func (d *decoder) unmarshalBool(value *unstable2.Node, v reflect.Value) error {
	b := value.Data[0] == 't'
	...
	default:
		return unstable2.NewParserError(value.Data, "cannot assign boolean to a %t", b)
	}

	return nil
}

func (d *decoder) unmarshalFloat(value *unstable2.Node, v reflect.Value) error {
	...
	case reflect.Float32:
		if f > math.MaxFloat32 {
			return unstable2.NewParserError(value.Data, "number %f does not fit in a float32", f)
		}
		v.SetFloat(f)
	case reflect.Interface:
		v.Set(reflect.ValueOf(f))
	default:
		return unstable2.NewParserError(value.Data, "float cannot be assigned to %s", v.Kind())
	}
...
func (d *decoder) unmarshalInteger(value *unstable2.Node, v reflect.Value) error {
	kind := v.Kind()
	...
	case reflect.Interface:
		r = reflect.ValueOf(i)
	default:
		return unstable2.NewParserError(d.p.Raw(value.Raw), d.typeMismatchString("integer", v.Type()))
	}
...
func (d *decoder) unmarshalString(value *unstable2.Node, v reflect.Value) error {
	switch v.Kind() {
	case reflect.String:
	...
	default:
		return unstable2.NewParserError(d.p.Raw(value.Raw), d.typeMismatchString("string", v.Type()))
	}

	return nil
}

func (d *decoder) handleKeyValue(expr *unstable2.Node, v reflect.Value) (reflect.Value, error) {
...
func (d *decoder) handleKeyValueInner(key unstable2.Iterator, value *unstable2.Node, v reflect.Value) (reflect.Value, error) {
	if key.Next() {
		// Still scoping the key
...
func (d *decoder) handleKeyValuePart(key unstable2.Iterator, value *unstable2.Node, v reflect.Value) (reflect.Value, error) {
